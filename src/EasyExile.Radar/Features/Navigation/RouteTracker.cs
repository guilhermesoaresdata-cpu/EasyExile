...
// Ported verbatim from POE2Radar (MIT), Overlay/Navigation/RouteTracker.cs.
// Every threshold, every trigger and the progressive cursor rule are the
// reference's; the comments explaining WHY each one exists are its own, and they
...
/// <c>BackgroundReplanner</c> worker; this tracker only ever reads its own waypoints. Owned by
