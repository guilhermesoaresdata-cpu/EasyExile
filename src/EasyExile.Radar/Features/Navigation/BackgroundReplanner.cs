...
// Ported from POE2Radar (MIT), Overlay/Navigation/BackgroundReplanner.cs. One
// worker, one planner, requests coalesced per target — which is what keeps A*
// off the render thread and stops a moving player backing up a queue of stale
// plans.
...
/// is immutable per area, so the worker reading the snapshotted <see cref="TerrainSnapshot"/> reference
...
public sealed class BackgroundReplanner : IDisposable
{
    ...
    public readonly record struct Request(
        string TargetId, TerrainSnapshot Terrain, (int X, int Y) Start, (int X, int Y) Goal);
